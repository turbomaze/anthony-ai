...
var Crush = (function() {
  ...
  return {
    ...
    drawLine: function(ctx, start, end, color, thickness) {
	  ...
	  getPixelsFromImage: function(
	  	imageLoc, width, callback, aspectRatioMultiplier, modifier
	  ) {
	    ...
	    img.onload = function() {
	      ...
	      tmpCanvas.height = width*aspectRatio;
	      tmpCanvas.style.display = 'none';
	      document.body.appendChild(tmpCanvas);
	      var tmpCtx = tmpCanvas.getContext('2d');
	      tmpCtx.drawImage(img, 0, 0, width, width*aspectRatio);
	      ...
	      if (modifier != null) { 
	        imageData.data = modifier(imageData.data); 
	        tmpCtx.putImageData(imageData, 0, 0); 
	      }
	  
	      callback(
	  			imageData,
	  			new Date().getTime() - timeStartedGettingPixels
	  		);
	    };
	  
	    img.src = imageLoc; //load the image
	  },
